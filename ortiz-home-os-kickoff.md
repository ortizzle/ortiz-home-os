...
- **v2.5 — the worldly manager (SHIPPED):** suggestions must clear a quality bar. Standing rules, learned the hard way:
...
- **v3 — memory, meals, and the kids (SHIPPED):** three capabilities that make the manager feel like it's actually running the house:
...
- **v4 — streamline (CURRENT; from the July 2026 usefulness review with Chris & Kat):** the app had grown wider than two users need. **The goal going forward is not more features — it's making the proven core (calendar, tasks, daily brief, weekly rhythm) accurate, low-friction, and trustworthy.** Every proposed feature has to clear these standing rules:
  - **One list.** Plans merged into Tasks (v68): a "plan" is a dateless task under Someday; multi-step work is **subtasks on the task**, not prose or a second list. Never reintroduce a parallel capture surface — everything lands in Tasks, Calendar, Grocery, or a meeting agenda.
  - **AI output must land somewhere durable and checkable.** The essay deep-dive became "✨ Break it down" → subtasks (v68). The one surviving write-up is the decision aid on Claudia's open questions — questions are where prose earns its cost. Don't add AI features whose output is read once and lost; structured beats eloquent, and it's cheaper.
  - **Settled means settled.** "✓ Not needed" logs a dismissal that suppresses the suggestion ~3 weeks (then quietly expires); meeting decisions feed every prompt for 21 days; resolved questions aren't re-asked while their log entry lives. Claudia re-raising settled things is the #1 trust-killer — every new prompt must receive and respect these blocks.
  - **Memory is standing facts, not stale answers.** The memory store (v55) holds lean, typed, per-fact-editable facts fed to every prompt. Answers to review questions become memory only via the explicit "Remember this" opt-in (default off). Never auto-promote a one-time decision into permanent memory — that's how memory went stale the first time.
  - **Attribution over guessing.** Events carry their source Google calendar (v69): [Family] = shared, a parent-named calendar = that parent's commitment; cross-calendar duplicates resolve Family-first. When the AI keeps guessing wrong, prefer feeding it real signal over adding "don't guess" prompt rules.
  - **Fewer decision points.** A suggestion row gets one obvious destination per type (+ Task / + Calendar / + Grocery / → Meeting / Not needed). Any new button must replace a decision, not add one.
  - **Cost tracks value.** The brief stays fast and search-free; the review does the worldly work; breakdowns are small structured calls. When a feature "doesn't feel worth the cost," fix what's being asked of the model before removing the feature.

## Coordination (multiple Claude sessions ship here)

Parallel Claude Code sessions push to this repo. Before building: `git fetch` and start from `origin/main`, read this file's CURRENT phase, and check the Settings version label for what's deployed. When shipping: bump `APP_VERSION` (app.js) and the `CACHE` name (sw.js) together, with a word or two on what shipped.
